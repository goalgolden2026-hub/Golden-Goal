...
export async function PUT(request) {
    try {
        ...
        if (action === 'CHANGE') {
            if (balance < 5000) {
                return NextResponse.json({ success: false, error: "Insufficient Token Balance (5.000 $GoldenGoal required)" }, { status: 403 });
            }
            if (!newPrediction) {
                return NextResponse.json({ success: false, error: "New prediction is required" }, { status: 400 });
            }
            
            // Log 5.000 tokens (2.500 BURN, 2.500 REWARD_POOL)
            await sql`INSERT INTO treasury_logs ("walletAddress", amount, type) VALUES (${walletAddress}, 2500, 'BURN_CHANGE_PREDICTION')`;
            await sql`INSERT INTO treasury_logs ("walletAddress", amount, type) VALUES (${walletAddress}, 2500, 'REWARD_POOL_CHANGE_PREDICTION')`;

            // Update prediction
            await sql`UPDATE predictions SET prediction = ${newPrediction}, "updatedAt" = CURRENT_TIMESTAMP WHERE id = ${finalPredictionId}`;
            
            return NextResponse.json({ success: true, message: "Prediction updated successfully. 5.000 $GoldenGoal deducted." });
            ...
            await sql`INSERT INTO treasury_logs ("walletAddress", amount, type) VALUES (${walletAddress}, 5000, 'BURN_CANCEL_PREDICTION')`;
            await sql`INSERT INTO treasury_logs ("walletAddress", amount, type) VALUES (${walletAddress}, 5000, 'REWARD_POOL_CANCEL_PREDICTION')`;
            ...
            return NextResponse.json({ success: true, message: "Prediction canceled successfully. 10.000 $GoldenGoal deducted and daily quota restored." });
        } else {
